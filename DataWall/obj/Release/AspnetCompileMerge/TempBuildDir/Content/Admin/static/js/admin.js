...
layui.define(['jquery', 'form', 'layer', 'element'], function(exports) {
	...
	var tab = {
		tabAdd: function(title, url, id) {
            //判断当前id的元素是否存在于tab中
            //console.log($("#WeTabTip li[lay-id=" + id + "]"));
                ...
                if (XMLHttpRequest.responseJSON != null) {
                    switch (XMLHttpRequest.responseJSON.code) {
                        case '401':
                            ...
                            break;
                        case '402':
                            layer.alert(XMLHttpRequest.responseJSON.msg, { icon: 5 }, function () {
                                //跳转到登录页
                                top.location.href = XMLHttpRequest.responseJSON.url;
                            });
                            break;
                        default:
                            if (title == null || title == '') {
