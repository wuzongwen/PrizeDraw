...
namespace DataWall.WeCaht
{
    ...
    public partial class CustomMessageHandler
    {
        ...
        {
            return string.Format(@"欢迎关注上业科技抽奖平台");
        }
            ...
            try
            {
                ...
                using (PrizeDrawContext db = new PrizeDrawContext())
                {
                    ...
                    if (PrizeDrawUser == null)
                    {
                        ...
                        SysPrizeDrawUserId = sysPrizeDrawUser.ID;
                    }
                    else
                    {
                        SysPrizeDrawUserId = PrizeDrawUser.ID;
                    }

                    string PrizeDrawCode = requestMessage.ScanCodeInfo.ScanResult.Substring(requestMessage.ScanCodeInfo.ScanResult.IndexOf("=") + 1);
                    LogHelper.InfoLog(PrizeDrawCode);
                    PrizeDrawCode = SecurityHelper.DecryptDES(PrizeDrawCode, CookieSecretKey);
                    LogHelper.InfoLog(PrizeDrawCode);
                    SysLibrary sysLibrary = db.SysLibrarys.AsNoTracking().FirstOrDefault(l => l.LibraryCode == PrizeDrawCode);
                    if (db.SysPrizeDrawRecords.FirstOrDefault(s => s.SysPrizeDrawUserId == SysPrizeDrawUserId & s.SysLibraryId == sysLibrary.ID) == null)
                    {
                        SysPrizeDrawRecord sysPrizeDrawRecord = new SysPrizeDrawRecord
                        {
                            SysPrizeDrawUserId = SysPrizeDrawUserId,
                            SysLibraryId = sysLibrary.ID,
                            State = 0,
                            CrateTime = DateTime.Now,
                            EditTime = DateTime.Now
                        };
                        db.SysPrizeDrawRecords.Add(sysPrizeDrawRecord);
                        db.SaveChanges();
                        responseMessage.Content = "参与成功;回复【CY】查询参与的抽奖活动,【CX】查询中奖状态";
                    }
                    else
                    {
                        responseMessage.Content = "您已参与本次抽奖活动，无需重复参加;回复【CY】查询参与的抽奖活动,【CX】查询中奖状态";
                    }
                }
                return responseMessage;
            }
        ...
        public override IResponseMessageBase OnEvent_ScanRequest(RequestMessageEvent_Scan requestMessage)
        {
            ...
            responseMessage.Content = "您的openid为:" + requestMessage.FromUserName + ";二维码参数" + requestMessage.EventKey;
